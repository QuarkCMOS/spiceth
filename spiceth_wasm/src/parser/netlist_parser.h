#pragma once
/**
 * netlist_parser.h  –  WASM/web-compatible netlist parser.
 *
 * CHANGES from original:
 *   - parse_netlist(string filename) → parse_netlist_string(string content)
 *   - No <fstream>, no filesystem access, no OS calls.
 *   - parse_netlist(path) shim is intentionally REMOVED; callers must pass
 *     the netlist text directly (read on the JS side, passed via Emscripten).
 */
...
// NOTE: <fstream> is intentionally NOT included.
// No regex either (not needed and heavy on WASM).
...
inline bool looks_like_component(const std::vector<std::string>& tokens)
{
    ...
    if (prefix == 'R' || prefix == 'C' || prefix == 'L') return n == 4;
    if (prefix == 'V' || prefix == 'I') return n >= 4;
    if (prefix == 'G' || prefix == 'E') return n == 6;
    if (prefix == 'F' || prefix == 'H') return n == 5;
    if (prefix == 'D') return n == 4;
    return false;
}

// ─── Transient signal builders ────────────────────────────────────────────────
...
inline FunctionCall parse_function(const std::string& text)
{
    ...
    std::string tok;
    while (ss >> tok) f.args.push_back(tok);
    return f;
}

inline double get_arg(const std::vector<std::string>& args, size_t idx, double dv)
{
    return (idx >= args.size()) ? dv : parse_value(args[idx]);
}

inline TransientSignal parse_pulse(const FunctionCall& f)
{
    if (f.args.size() < 2)
        throw std::runtime_error("PULSE requires at least 2 args: PULSE(V1 V2 [TD TR TF PW PER])");
    PulseParams p;
...
{
    if (f.args.size() < 3)
        throw std::runtime_error("SIN requires at least 3 args: SIN(VO VA FREQ [TD THETA PHASE])");
    SinParams s;
                                    ...
                                    size_t start)
{
    ...
    if (tokens.size() == start + 1) {
        ...
        if      (f.name == "sin")   spec.transient = parse_sin(f);
        else if (f.name == "pulse") spec.transient = parse_pulse(f);
        else throw std::runtime_error(name + ": unsupported source function '" + f.name + "'");
        return spec;
    }
...
// ─── Main parser (string-based, no filesystem) ───────────────────────────────

/**
 * Parse a SPICE-like netlist from a string (netlist content, not a filename).
 * This is the WASM-safe entry point.
 */
inline Circuit parse_netlist_string(const std::string& content)
{
    Circuit circuit;
    ...
    while (std::getline(stream, line)) {
        ...
        // Skip SPICE title card (first non-comment, non-directive line that
        // doesn't look like a component)
        ...
        if (name[0] == '.') {
            ...
            if (dir == ".ac") {
                ...
                if (tokens.size() != 5)
                    throw std::runtime_error("Invalid .DC syntax: .DC SRC START STOP STEP");
                ...
                if (paren_pos != std::string::npos) {
                    ...
                    std::string flat;
                    for (size_t i = 2; i < tokens.size(); ++i) flat += " " + tokens[i];
                ...
                for (const auto& pt : param_tokens) {
                    ...
                    if (!val.empty()) params[key] = parse_value(val);
                }
                if (mtype == "D") {
                    DiodeModel dm;
                    dm.name = mname;
            ...
            // ignore unknown directives silently (.probe, .save, .param, ...)
            continue;
        }
